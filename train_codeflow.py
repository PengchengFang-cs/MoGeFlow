...
from data.t2m_dataset import Text2MotionDataset, Text2MotionDatasetEval, collate_fn
from models.codeflow import CodeFlowEvalConfig, MotionCodeFlow, MotionCodeFlowConfig, evaluate_codeflow_t2m
from models.codeflow.motion_code_flow import lengths_to_mask, sample_timesteps
...
def make_config(opt) -> MotionCodeFlowConfig:
    return MotionCodeFlowConfig(
        ...
        code_ce_t_min=opt.code_ce_t_min,
        code_ce_t_max=opt.code_ce_t_max,
        code_ce_gamma=opt.code_ce_gamma,
        code_ce_normalize=opt.code_ce_normalize,
        flow_loss_weight=opt.flow_loss_weight,
        terminal_loss_weight=opt.terminal_loss_weight,
        clean_loss_weight=opt.clean_loss_weight,
    )
...
def load_checkpoint(path: Path, model: MotionCodeFlow, optimizer, scaler):
...
def build_full_eval_components(opt, device):
    ...
    loader = DataLoader(
        dataset,
        batch_size=opt.full_eval_batch_size,
        num_workers=opt.full_eval_num_workers,
        drop_last=True,
...
def run_full_eval(
    ...
    try:
        ...
        eval_cfg = CodeFlowEvalConfig(
            ...
            terminal_mode=None,
            decode_mode=getattr(opt, "decode_mode", None),
        ...
        payload: Dict[str, object] = {
            ...
            "decode_mode": str(getattr(opt, "decode_mode", "nearest")),
...
def main(
    ...
    ema = make_ema(model)
    if opt.resume:
        start_epoch, global_step, loaded_ema = load_checkpoint(Path(opt.resume), model, optimizer, scaler)
    ...
    for epoch in range(start_epoch, opt.max_epoch):
        ...
        for batch in train_loader:
            ...
            if global_step % opt.log_every == 0:
                ...
                metrics.update({"epoch": epoch, "step": global_step, "lr": lr})
