#!/usr/bin/env python3
"""Render a static Quaternius humanoid-character panel from HumanML3D joints.

Run inside Blender:
  blender -b Warrior.blend --python tools/render_quaternius_retarget_panel.py -- ...
"""
...
def main_armature() -> bpy.types.Object:
    ...
    if not armatures:
        raise RuntimeError("No armature found in the character blend file")
    return armatures[0]
...
def apply_pose(arm: bpy.types.Object, frame: np.ndarray) -> None:
    # HumanML3D indices.
    root = frame[0]
    ...
    orient_bone(arm, "Root", np.asarray([root[0], root[1], 0.0], dtype=np.float32), root)
    orient_bone(arm, "Body", root, spine1)
    orient_bone(arm, "Hips", hips_mid, spine1)
    orient_bone(arm, "Abdomen", spine1, spine2)
    orient_bone(arm, "Torso", spine2, neck)
    orient_bone(arm, "Neck", neck, head)
    orient_bone(arm, "Head", head, head_tail(head, neck))

    orient_bone(arm, "Shoulder.L", neck, l_shoulder)
    orient_bone(arm, "UpperArm.L", l_shoulder, l_elbow)
    orient_bone(arm, "LowerArm.L", l_elbow, l_wrist)
    orient_bone(arm, "Fist.L", l_wrist, hand_tail(l_wrist, l_elbow))

    orient_bone(arm, "Shoulder.R", neck, r_shoulder)
    orient_bone(arm, "UpperArm.R", r_shoulder, r_elbow)
    orient_bone(arm, "LowerArm.R", r_elbow, r_wrist)
    orient_bone(arm, "Fist.R", r_wrist, hand_tail(r_wrist, r_elbow))

    orient_bone(arm, "UpperLeg.L", l_hip, l_knee)
    orient_bone(arm, "LowerLeg.L", l_knee, l_ankle)
    orient_bone(arm, "Foot.L", l_ankle, l_foot)
    orient_bone(arm, "UpperLeg.R", r_hip, r_knee)
    orient_bone(arm, "LowerLeg.R", r_knee, r_ankle)
    orient_bone(arm, "Foot.R", r_ankle, r_foot)
...
def configure_render(width: int, height: int) -> None:
    ...
    if scene.world is None:
        scene.world = bpy.data.worlds.new("quaternius_panel_world")
    scene.world.color = (0.982, 0.982, 0.962)


def scaled_frames(coords: np.ndarray, frame_ids: list[int], body_scale: float, path_scale: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i, frame in enumerate(selected):
        ...
        offsets[i] = np.asarray(
            [root[0] * path_scale, root[1] * path_scale, (root[2] - selected[:, 0, 2].mean()) * body_scale],
            dtype=np.float32,
        )
...
def main() -> None:
    ...
    frames, offsets = scaled_frames(coords, frame_ids, auto_body_scale * float(args.body_scale), float(args.path_scale))

    arm = main_armature()
    state = save_pose_state(arm)
    baked_collection = bpy.data.collections.new("character_baked_poses")
