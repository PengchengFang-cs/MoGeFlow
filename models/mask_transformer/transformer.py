...
class MaskTransformer(nn.Module):
    ...
    def generate(self,
        ...
        if self.cond_mode == 'text':
            ...
            cond_vector = self.enc_action(conds).to(device).float()
        elif self.cond_mode == 'uncond':
...
class ResidualTransformer(nn.Module):
    def __init__(self, code_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8, cond_drop_prob=0.1,
        ...
        self.num_actions = kargs.get('num_actions', 1)
        self.cond_drop_prob = cond_drop_prob
        self.res_use_uni_mask = getattr(opt, 'res_use_uni_mask', False)
        self.res_uni_solver_steps = max(1, int(getattr(opt, 'res_uni_solver_steps', 16)))
        self.res_uni_path_a = float(getattr(opt, 'res_uni_path_a', 0.9))
        self.res_uni_path_c = float(getattr(opt, 'res_uni_path_c', 3.0))
        self.res_uni_t_min = 1e-6
        self.res_uni_t_max = 1.0 - 1e-6
        ...
        if self.cond_mode == 'text':
            ...
            self.clip_version = clip_version
            self.clip_model = self.load_and_freeze_clip(clip_version)
        if self.res_use_uni_mask:
            print(
                'ResidualTransformer: FUDOKI-style uni-mask is enabled '
                f'(steps={self.res_uni_solver_steps}, a={self.res_uni_path_a}, c={self.res_uni_path_c}).'
            )
    ...
    def _gather_active_codes(self, active_vocab_embed, token_ids):
        token_ids = token_ids.clamp(min=0, max=self.opt.num_tokens - 1)
        gather_indices = repeat(token_ids, 'b n -> b n d', d=active_vocab_embed.shape[-1])
        return active_vocab_embed.gather(1, gather_indices)

    def _metric_prob_from_target(self, emb_x_1, active_vocab_embed, t):
        emb_x_1 = F.normalize(emb_x_1, p=2, dim=-1)
        vocab_embed = F.normalize(active_vocab_embed, p=2, dim=-1)

        cosine = torch.einsum('bnd,bcd->bnc', emb_x_1, vocab_embed)
        distance_x_1_to_all = (2.0 - 2.0 * cosine).clamp(min=0.0).pow(2)

        t = t.clamp(min=self.res_uni_t_min, max=self.res_uni_t_max)
        beta_t = self.res_uni_path_c * ((t / (1.0 - t)) ** self.res_uni_path_a)
        beta_t = beta_t.view(-1, 1, 1)
        prob_x_t = torch.softmax(-beta_t * distance_x_1_to_all, dim=-1)
        return prob_x_t, distance_x_1_to_all

    def _metric_d_beta(self, t):
        safe_t = t.clamp(min=self.res_uni_t_min, max=self.res_uni_t_max)
        ratio = safe_t / (1.0 - safe_t)
        d_beta_t = self.res_uni_path_c * self.res_uni_path_a * (ratio ** (self.res_uni_path_a - 1.0)) / ((1.0 - safe_t) ** 2)
        return torch.where(t <= 0.0, torch.zeros_like(d_beta_t), d_beta_t)

    def apply_uni_mask_corruption(self, x_1_ids, non_pad_mask, active_vocab_embed):
        """
        FUDOKI metric-induced discrete corruption:
          x_t ~ p_t(. | x_1),  p_t(x|x_1) ∝ exp(-beta_t * d(x_1, x))
        """
        bs = x_1_ids.shape[0]
        device = x_1_ids.device
        t = uniform((bs,), device=device)

        emb_x_1 = self._gather_active_codes(active_vocab_embed, x_1_ids)
        prob_x_t, _ = self._metric_prob_from_target(emb_x_1, active_vocab_embed, t)
        x_t_ids = torch.multinomial(prob_x_t.reshape(-1, prob_x_t.shape[-1]), num_samples=1).view_as(x_1_ids)
        x_t_ids = torch.where(non_pad_mask, x_t_ids, self.pad_id)
        return x_t_ids

    @torch.no_grad()
    def _fudoki_refine_one_layer(self, history_sum, q_id, cond_vector, padding_mask, temperature, cond_scale, topk_filter_thres):
        """
        FUDOKI-style iterative CTMC refinement for one residual quantizer layer.
        Tokens are always updatable until the final step.
        """
        device = history_sum.device
        bs, seq_len = history_sum.shape[:2]

        active_vocab_embed = self.token_embed_weight[q_id - 1, :self.opt.num_tokens]
        active_vocab_embed = repeat(active_vocab_embed, 'c d -> b c d', b=bs)

        x_t = torch.randint(0, self.opt.num_tokens, (bs, seq_len), device=device)
        x_t = torch.where(padding_mask, torch.full_like(x_t, self.pad_id), x_t)

        time_grid = torch.linspace(0.0, 1.0, self.res_uni_solver_steps + 1, device=device)
        temp = max(float(temperature), 1e-5)

        for step in range(self.res_uni_solver_steps):
            t_scalar = time_grid[step]
            h_scalar = time_grid[step + 1] - time_grid[step]

            emb_x_t = self._gather_active_codes(active_vocab_embed, x_t)
            motion_codes = history_sum + emb_x_t

            logits = self.forward_with_cond_scale(
                motion_codes, q_id, cond_vector, padding_mask, cond_scale=cond_scale
            ).permute(0, 2, 1)[..., :self.opt.num_tokens]
            if topk_filter_thres < 1.0:
                logits = top_k(logits, topk_filter_thres, dim=-1)

            prob_x_1_t = torch.softmax(logits / temp, dim=-1)
            x_1 = Categorical(prob_x_1_t).sample()
            x_1 = torch.where(padding_mask, torch.full_like(x_1, self.pad_id), x_1)

            if step == self.res_uni_solver_steps - 1:
                x_t = x_1
                break

            t_batch = torch.full((bs,), float(t_scalar.item()), device=device)
            emb_x_1 = self._gather_active_codes(active_vocab_embed, x_1)
            prob_x_t, distance_x_1_to_all = self._metric_prob_from_target(emb_x_1, active_vocab_embed, t_batch)

            emb_x_t_norm = F.normalize(emb_x_t, p=2, dim=-1)
            emb_x_1_norm = F.normalize(emb_x_1, p=2, dim=-1)
            distance_x_t_to_x_1 = (2.0 - 2.0 * (emb_x_t_norm * emb_x_1_norm).sum(dim=-1, keepdim=True)).clamp(min=0.0).pow(2)
            distance = F.relu(distance_x_t_to_x_1 - distance_x_1_to_all)

            d_beta_t = self._metric_d_beta(t_batch).view(-1, 1, 1)
            u = prob_x_t * d_beta_t * distance
            u = u.scatter(2, x_t.clamp(max=self.opt.num_tokens - 1).unsqueeze(-1), 0.0)
            u = torch.where(padding_mask.unsqueeze(-1), torch.zeros_like(u), u)

            intensity = u.sum(dim=-1)
            jump_prob = 1.0 - torch.exp(-h_scalar * intensity)
            mask_jump = (torch.rand_like(jump_prob) < jump_prob) & (~padding_mask) & (intensity > 1e-12)

            if mask_jump.any():
                u_jump = u[mask_jump]
                u_jump = u_jump / u_jump.sum(dim=-1, keepdim=True).clamp(min=1e-12)
                x_next = Categorical(u_jump).sample()
                x_t = x_t.clone()
                x_t[mask_jump] = x_next
    ...
    def forward(self, all_indices, y, m_lens):
        ...
        motion_codes = history_sum

        if self.res_use_uni_mask:
            active_vocab_embed = self.token_embed_weight[active_q_layers - 1, :self.opt.num_tokens]  # (b, c, d)
            noised_active_indices = self.apply_uni_mask_corruption(active_indices, non_pad_mask, active_vocab_embed)
            noised_active_codes = self._gather_active_codes(active_vocab_embed, noised_active_indices)
            motion_codes = motion_codes + noised_active_codes
        ...
        logits = self.trans_forward(motion_codes, active_q_layers, cond_vector, ~non_pad_mask, force_mask)
    ...
    def generate(self,
        ...
        for i in range(1, num_quant_layers):
            ...
            history_sum += token_embed.gather(1, gathered_ids)
            if self.res_use_uni_mask:
                pred_ids = self._fudoki_refine_one_layer(
