...
def main(
    ...
    if tokenizer_downsample is not None and int(getattr(opt, "unit_length", 4)) != int(tokenizer_downsample):
        raise ValueError(
            f"unit_length={int(getattr(opt, 'unit_length', 4))} does not match "
            f"tokenizer downsample_factor={int(tokenizer_downsample)}"
        )
    ...
    eval_cfg = CodeFlowEvalConfig(
        ...
        include_code_metrics=(
            not args.disable_code_metrics
            and bool(getattr(model.tokenizer, "supports_code_metrics", True))
        ),
