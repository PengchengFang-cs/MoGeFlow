...
（无）09-05 B11/B12 收尾；1476669 上 enclat 训练跑完 600ep 后可释放
...
| 日期 | ID | 任务 | 结论一句话 | 详情 |
|---|---|---|---|---|
| 09-05 | B11 | 目标空间消融归一化修正（encoder-latent 改用自身统计量重训） | FID 0.151→**0.082**、Top3 0.860→0.853；格点目标仍两轴皆胜，攻击点消除；已入 Table 3(b) | LOG#B11 |
| 09-04 | B12 | guidance 扫描（六月检查点，cfg 1–8，带 MM） | 曲线平滑、cfg 6 位置合理、FID 最低 0.0495；**裁定不进论文**（属防御性） | LOG#B12 |
| 09-02 | B10 | MultiModality 补测（两数据集）+ v 时代检查点评测通路 | HML3D 1.216 / KIT 0.947，均为各自列最低；v 时代权重需 `--head_is_velocity` 否则静默塌成噪声 | LOG#B10 |
| 08-31 | B9 | 审稿回应诊断三条（解码侧 ρ / 活跃码配平 ρ / 离格偏移 in-span） | 解码侧 0.834 > 编码侧 0.822；配平到 37 码仅掉 0.0017；in-span 3.52× 随机基线——三条全为正面结果 | LOG#B9 |
| 08-20 | 论文 | 三审稿人评审（3/3/4 分）→ 按用户裁定改稿：全线 CLIP 表述、删 8 处防御性写作、正文 12→9 页、几何诊断与目标空间消融并排、定性图回正文 | Overleaf `4743d5a`，编译 0 错误/0 溢出 | 见 PLAN 决策登记 |
...
| 08-20 | B4b | 第二批 100 seed（常用 + 随机三/四/五位数），cfg10 | 0.0680/0.8670；四组无系统性差异；与第一批（0.0676/0.8671）一致 | LOG#B4b |
